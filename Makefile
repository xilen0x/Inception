...
# COMPOSE     = $(SRCS)/docker-compose.yml
# HOST_URL    = castorga.42.fr
# DATA_DIR    = /home/$(USER)/data  # Ruta absoluta, evitando posibles problemas con $(HOME)

# # Rules -----------------------------------------------------------------------

# # Regla principal que inicia los contenedores y configura el host
# all: up

# up: create_dir
# 	@grep -qxF "127.0.0.1 $(HOST_URL)" /etc/hosts || sudo sh -c "echo '127.0.0.1 $(HOST_URL)' >> /etc/hosts"
# 	@docker compose -p $(NAME) -f $(COMPOSE) up -d --build
# 	@echo "$(NAME) containers started"
...
# 	@if [ -n "$(DATA_DIR)" ]; then sudo rm -rf $(DATA_DIR); fi
# 	@echo "Cleaned data directory and removed containers"
